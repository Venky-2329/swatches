...
export function ApprovedUserForm(props: ApprovedUserFormProps) {
  ...
  const [fileList, setFileList] = useState<any>(
    props.isUpdate
      ? [
          {
            name: props.data.signImageName,
            status: 'done',
            url: props.data.signImageName,
          },
        ]
      : []
  );
  ...
  //   if (props.data) {
  //     const updateImage =
  //       'http://localhost/DELIVERY_CHALAN/upload-files' +
  //       props.data.signImageName;
  ...
  const uploadFieldProps: UploadProps = {
    ...
    beforeUpload: (file: any) => {
      if (
        !file.name.match(
          /\.(png|jpeg|PNG|jpg|JPG|pjpeg|gif|tiff|x-tiff|x-png)$/
        )
      ) {
    ...
    accept:
      'image/jpeg,image/jpg,image/png,image/gif,image/tiff,image/x-tiff,image/x-png',
    progress: {
      strokeColor: {
        '0%': '#108ee9',
        '100%': '#87d068',
              ...
              <Row gutter={24}>
                ...
                >
                  <Form.Item
                    ...
                    initialValue={
                      props.isUpdate ? props.data.signImageName : ''
                    }
                  >
                    {!imageUrl ? (
                      <Upload
