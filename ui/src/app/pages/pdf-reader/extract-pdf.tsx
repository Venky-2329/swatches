...
import { BOLD_HEIGHT_1, BOLD_HEIGHT_2, EMP_STR_EXP, UNWANTED_TEXT_1} from './pdf-regex-expressions';
...
export const PdfDataExtractor = async (pdf) => {
  ...
  for (let i = 1; i < pdf.numPages; i++) {
    ...
    const pageContent = textContent.items.filter((val, index) => {
      if(val.str === 'POMs'){startFlag= true;}
      if (endFlag) {
        startFlag = false;
        endFlag = false;
      }
      if(val.str === 'Nail Apron Total Width - at Bottom') {endFlag = true}
  ...
  console.log(code,product,supplQuote,uom,placement,brnBrownColor,brnQtyByColor,blkBlack)
