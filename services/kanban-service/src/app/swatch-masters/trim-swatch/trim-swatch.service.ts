...
export class TrimSwatchService {
  ...
  async getDataById(req: TrimSwatchStatus): Promise<CommonResponseModel> {
    try {
      ...
      ts.supplier_id AS supplierId,s.supplier_name , ts.trim_swatch_id , ts.trim_swatch_number , ts.po_number , ts.item_no , ts.item_description, 
      ts.invoice_no , ts.style_no , ts.grn_number , ts.grn_date , tue.file_name , tue.file_path ,ts.status,ts.created_at as createdAt,ts.rejection_reason ,ts.rework_reason as reworkReason , ts.approval_reason as approvalReason, ts.created_user as createdUser,ts.created_user_mail as createdUserMail,ts.rework,ts.remarks, se.employee_id AS employeeId , se.employee_name AS employeeName , ts.updated_at AS updatedAt , tue.upload_id AS uploadId
      FROM trim_swatch ts
      LEFT JOIN swatch_buyer b ON b.buyer_id = ts.buyer_id
      LEFT JOIN swatch_supplier s ON s.supplier_id = ts.supplier_id
      LEFT JOIN swatch_approval_users sau ON sau.approved_id = ts.approver_id
  ...
  async getAllApprovaTrimlUser(): Promise<CommonResponseModel> {
    let query = `SELECT  a.approved_id AS approvedId , a.user_id AS approvedUserId , a.email_id AS emailId , se.employee_name AS approvedUserName , a.is_active AS isActive
    FROM trim_swatch t
    LEFT JOIN swatch_approval_users a ON a.approved_id = t.approver_id
